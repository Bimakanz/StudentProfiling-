...
// ── Portfolio Card ────────────────────────────────────────────────────────────
const PortfolioCard = React.memo(({ item, onPress }: { item: any, onPress: () => void }) => {
  ...
  return (
    <View style={{
      ...
      <View style={{
        ...
        {item.thumbnail ? (
          isImage ? (
            <LazyImage uri={item.thumbnail} style={{ width: '100%', height: '100%' }} />
          ) : (
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 12, color: '#94a3b8', marginTop: 6 }}>Tidak ada thumbnail</Text>
          </View>
        )}
        ...
        <View style={{
          ...
          <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 10, color: '#000' }}>
            {item.nilai} GRADED
          </Text>
        </View>
      </View>
      ...
      {item.user && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 12 }}>
          <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 12, color: '#64748b' }}>{item.user.name}</Text>
        </View>
      )}
      ...
      >
        ...
        <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 12, color: '#000', letterSpacing: 1 }}>VIEW PROJECT</Text>
      </TouchableOpacity>
    </View>
  );
});
...
const PortfolioDetail = ({ item, user, onClose, onDelete, onEdit }: any) => {
  const isImage = item.thumbnail && /\.(jpeg|jpg|gif|png|webp)$/i.test(item.thumbnail);
  const formattedDate = new Date(item.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '.');
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
          <View style={{ borderWidth: 2, borderColor: '#000', backgroundColor: '#FF7F50', paddingHorizontal: 12, paddingVertical: 6 }}>
            <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 10, color: '#fff', textTransform: 'uppercase' }}>
              {user?.jurusan || item.kategori || 'ENGINEERING'}
            </Text>
          </View>
        ...
        <View style={{ gap: 12 }}>
          <TouchableOpacity style={{
            backgroundColor: '#FF7F50', borderWidth: 3, borderColor: '#000', paddingVertical: 14,
            ...
            <Text style={{ fontFamily: 'Inter_900Black', fontSize: 12, color: '#000', letterSpacing: 1 }}>LIHAT PROJECT LIVE</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={{
            ...
            <Text style={{ fontFamily: 'Inter_900Black', fontSize: 12, color: '#000', letterSpacing: 1 }}>UNDUH LAPORAN (PDF)</Text>
          </TouchableOpacity>
        </View>
...
export default function Portfolio() {
  ...
  const fetchPortfolios = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/portofolio`, {
        headers: { Accept: 'application/json' },
      });
      const json = await res.json();
      const all: any[] = json.data || [];

      // Filter hanya portofolio milik user yang login
  ...
  useFocusEffect(useCallback(() => { 
    ...
    const checkFlashMessage = async () => {
      const msg = await AsyncStorage.getItem('portfolio_toast');
      if (msg) {
        setSelectedPortfolio(null); // Force back to main list
        showToast(msg);
        await AsyncStorage.removeItem('portfolio_toast');
      }
    };
    checkFlashMessage();
  }, [fetchPortfolios]));
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/portofolio/${portfolioToDelete}`, {
        method: 'DELETE',
        headers: { Accept: 'application/json' }
      });
      ...
      if (data.status === 'success') {
        ...
        if (Platform.OS === 'web') window.alert("Gagal: " + (data.message || "Gagal menghapus portofolio."));
        else Alert.alert("Gagal", data.message || "Gagal menghapus portofolio.");
      }
    } catch (error) {
  ...
  const handleEdit = (item: any) => {
    // Navigasi ke TambahPortofolio sambil bawa data ID agar dihandle sebagai edit
    router.push({ pathname: '/TambahPortofolio', params: { id: item.id } } as any);
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#ffffff' }}>
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 13, color: '#000', letterSpacing: 1 }}>PORTFOLIO_SYSTEM</Text>
        </View>
        <View style={{
      ...
      {selectedPortfolio ? (
        ...
        <>
          ...
          <View style={{ backgroundColor: '#fff', paddingVertical: 16, alignItems: 'center' }}>
            <Text style={{ fontFamily: 'Inter_900Black', fontSize: 34, color: '#000', letterSpacing: 1 }}>PORTFOLIO</Text>
          ...
          {loading ? (
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 12, paddingHorizontal: 32 }}>
              ...
              <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 15, color: '#94a3b8' }}>Belum ada portofolio</Text>
              <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 13, color: '#cbd5e1', textAlign: 'center' }}>
                Tekan tombol + untuk menambahkan portofolio baru.
              </Text>
            </View>
          ) : (
            <FlatList
              ...
              renderItem={({ item }) => <PortfolioCard item={item} onPress={() => setSelectedPortfolio(item)} />}
          ...
          <View style={{ position: 'absolute', bottom: 16, left: 20 }}>
            <TouchableOpacity
              activeOpacity={0.85}
              onPress={() => router.push('/TambahPortofolio' as any)}
              style={{
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          <View style={{ backgroundColor: '#ffffff', borderWidth: 4, borderColor: '#000', width: '100%', shadowColor: '#000', shadowOffset: { width: 8, height: 8 }, shadowOpacity: 1, shadowRadius: 0, elevation: 10 }}>
            ...
            <View style={{ backgroundColor: '#b91c1c', padding: 16, borderBottomWidth: 3, borderBottomColor: '#000', flexDirection: 'row', alignItems: 'center', gap: 14 }}>
              ...
              <Text style={{ fontFamily: 'Inter_900Black', fontSize: 22, color: '#ffffff', flex: 1, textTransform: 'uppercase' }}>Hapus Proyek?</Text>
            </View>
            
            {/* Body */}
            <View style={{ padding: 24 }}>
              <Text style={{ fontFamily: 'Inter_500Medium', fontSize: 14, color: '#0f172a', lineHeight: 22 }}>
                Apakah kamu yakin ingin menghapus proyek ini? Tindakan ini <Text style={{ fontFamily: 'Inter_900Black', textDecorationLine: 'underline' }}>tidak dapat dibatalkan</Text>.
              </Text>
              ...
              <View style={{ marginTop: 20, backgroundColor: '#f1f5f9', borderWidth: 3, borderColor: '#000000', paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                ...
                <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 13, color: '#0f172a' }} numberOfLines={1}>
                  {portfolios.find(p => p.id === portfolioToDelete)?.judul || 'Project'}
                </Text>
              </View>
              ...
              <View style={{ marginTop: 32, gap: 14 }}>
                <TouchableOpacity onPress={confirmDelete} activeOpacity={0.85} style={{ backgroundColor: '#253341', borderWidth: 2, borderColor: '#000', paddingVertical: 14, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8, shadowColor: '#000', shadowOffset: { width: 4, height: 4 }, shadowOpacity: 1, shadowRadius: 0, elevation: 4 }}>
                  ...
                  <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 13, color: '#fff', letterSpacing: 1 }}>HAPUS PROYEK</Text>
                </TouchableOpacity>
